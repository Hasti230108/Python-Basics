...
class EmergencySystem:
    ...
    def complete_emergency(self, emgy_id, amb_id):
        found = False
        for emgy in self.multiple_emgy:
            if emgy.emgy_id == emgy_id:
                for amb in self.multiple_amb:
                    if amb.amb_id == amb_id:
                        ...
                        print(f"Emergency: {emgy.what_emgy}")
                        print(f"Ambulance: {amb.amb_id}")
                        print(f"Driver: {amb.driver_name}")
                        print("Ambulance Status: Available")
                        found = True
                        return
    ...
    def display_available_ambulances(self):
        print("==== AVAILABLE AMBULANCE ====")
        found = False
        for amb in self.multiple_amb:
            if amb.status == "Available":
                print(f"Ambulance ID: {amb.amb_id}")
                print(f"Driver: {amb.driver_name}")
                print(f"Location: {amb.current_loc}")
                print(f"Status: {amb.status}")
                print()
                found = True
        if not found:
            print("No ambulances available!")
...
print("\n===== EMERGENCY COMPLETED =====")
system.complete_emergency(204, 101)

print("\n===== AVAILABLE AMBULANCES AFTER COMPLETION =====")
system.display_available_ambulances()
